...
2. Hagan Sadina Rahman (3321600005)
3. Haichal Ihza (3321600007)
4. Rifda Quratul 'Ain (3321600012)
5. Muhammad Dzalhaqi (3321600023)
6. Wiradhika Nur Fadhillah (3321600028)
...
with tab_eda:
  ...
  # showing plot
  if show_filter == "Yes":
    year_data = data_bulanan_ei[data_bulanan_ei["Bulan-Tahun"].apply(
        lambda x: x.year) == choosen_year]
    ...
    fig.add_trace(go.Bar(
        ...
        name='Defisit',
        marker_color='yellow',
        hovertemplate='<b>Tahun %s</b><br>' % choosen_year +
    ...
    if len(surplus_data) > 0:
      bar_fig.add_trace(go.Bar(
          ...
          '<i>Nilai Neraca Perdagangan</i>: $%{y:.2f} <br>' +
          '<i>Bulan</i>: ' +
          defisit_data['Bulan-Tahun'].apply(
              lambda x: x.strftime('%B')) + '<br>'
      ))
...
with tab_model:
  ...
  with predict_tab:
    ...
    fig.add_trace(go.Scatter(
        ...
        '<i>Bulan</i>: ' +
        data_bulanan_ei['Bulan-Tahun'].apply(
            lambda x: x.strftime('%B')) + '<br>'
    ))
